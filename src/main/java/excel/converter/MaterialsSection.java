...
public class MaterialsSection {
    ...
    private void writeMaterialsToXML(Element rootElement, Document doc) {
        Element materials = doc.createElement(Tag.MATERIALS_SECTION.toString());
        ...
        for (Row row : sheet) {
            if (row.getRowNum() > this.materialsStartRowIndex) {
                ...
                if (vendorCodeCell == null || attributeNameCell == null) {
                    // no need to do anything - there is no cell either vendor code or attribute
                    // description. Skip this row
                } else {
                    ...
                    if (vendorCodeCellValue == "" || attributeNameCellValue == "") {
                        ...
                        Element material = doc.createElement(Tag.MATERIAL.toString());
                        materials.appendChild(material);

                        // this row has a vendor code and the attribute name. We can write such row into
                        // the xml file
    ...
    public static MaterialsSection createMaterials(Element materialsElement) throws IOException {
        // create an instance of materialsSection. It actually does not have any
        // attributes and is just a holder for the vector of groups
        MaterialsSection materialsSection = new MaterialsSection();

        // get all the material groups elements
        NodeList materialGroupsList = materialsElement.getElementsByTagName(Tag.MATERIAL_GROUP.toString());
    ...
    public short writeToXLSX(Sheet mainSheet, short currentRow, Styles stylesCache) {
        ...
        // we have finished the output of all the groups. Now output the sum of all of
        // them
    ...
    private void writeSumFormula(Sheet mainSheet, short currentRow, Styles stylesCache) {
